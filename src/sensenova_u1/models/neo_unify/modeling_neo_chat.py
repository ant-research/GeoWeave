# Adapted from OpenSenseNova/SenseNova-U1 under Apache-2.0.
# Modified by the GeoWeave authors in 2026: interleave sampling, reproducible RNG,
# autoregressive entropy reporting, and inference controls.
...
class NEOChatModel(PreTrainedModel):
    ...
    def interleave_gen(
        ...
        def _generation_config_value(name, default):
            if generation_config is None:
                return default
            if isinstance(generation_config, dict):
                value = generation_config.get(name, default)
            else:
                value = getattr(generation_config, name, default)
            return default if value is None else value

        max_new_tokens = int(_generation_config_value('max_new_tokens', 8192))
        repetition_penalty = float(_generation_config_value('repetition_penalty', 1.0))
        do_sample = bool(_generation_config_value('do_sample', False))
        temperature = float(_generation_config_value('temperature', 1.0))
        top_p = float(_generation_config_value('top_p', 1.0))
        top_k = int(_generation_config_value('top_k', 0))
        if max_new_tokens <= 0:
        ...
        # Keep autoregressive sampling independent from diffusion noise. This
        # makes a candidate reproducible even if its generated image count or
        # diffusion implementation changes.
        text_generator = torch.Generator(self.device).manual_seed(seed)
        image_generator = torch.Generator(self.device).manual_seed(
            (int(seed) ^ 0x5DEECE66D) & ((1 << 63) - 1)
        )

        def next_text_token(logits):
            """Return the selected token and entropy of the raw model distribution.

            Entropy is computed in fp32 before repetition penalty, temperature,
            top-k, or top-p are applied. interleave_gen owns its token loop, so
            HuggingFace's generate() logits processors/warpers are not invoked
            automatically.
            """
            raw_entropy = None
            if record_ar_entropy:
                raw_scores = logits[:, -1, :].float()
                raw_log_probs = torch.log_softmax(raw_scores, dim=-1)
                raw_probs = raw_log_probs.exp()
                # The where avoids 0 * -inf if a backend ever emits an exact
                # zero probability. Only the scalar entropy survives here.
                raw_entropy = -torch.where(
                    raw_probs > 0,
                    raw_probs * raw_log_probs,
                    torch.zeros_like(raw_probs),
                ).sum(dim=-1)

            scores = logits[:, -1, :]
            if repetition_penalty != 1.0 and generated_token_history:
                token_ids = torch.tensor(
                    list(set(generated_token_history)),
                    dtype=torch.long,
                    device=scores.device,
                )
                repeated_scores = scores.index_select(1, token_ids)
                ...
                scores = scores.clone()
                scores.scatter_(1, token_ids.unsqueeze(0), repeated_scores)
            if not do_sample:
                return torch.argmax(scores, dim=-1), raw_entropy

            # Sampling in fp32 avoids bf16 softmax/multinomial precision loss.
            scores = scores.float() / temperature
            if top_k > 0:
                effective_top_k = min(top_k, scores.shape[-1])
                kth = torch.topk(scores, effective_top_k, dim=-1).values[..., -1, None]
                scores = scores.masked_fill(scores < kth, float('-inf'))
            if top_p < 1.0:
                sorted_scores, sorted_indices = torch.sort(scores, descending=True, dim=-1)
                sorted_probs = torch.softmax(sorted_scores, dim=-1)
                remove = torch.cumsum(sorted_probs, dim=-1) > top_p
                # Retain the first token crossing the nucleus threshold.
            ...
            next_token = torch.multinomial(
                probs, num_samples=1, generator=text_generator
            ).squeeze(-1)
            return next_token, raw_entropy
        ...
        def record_selected_token_entropy(token_id, entropy):
            nonlocal ar_entropy_sum_raw
            nonlocal ar_entropy_token_count
            nonlocal ar_entropy_excluded_token_count
            if not record_ar_entropy:
                return
            if token_id in excluded_ar_token_ids:
                ar_entropy_excluded_token_count += 1
                return
            ar_entropy_sum_raw += float(entropy.item())
            ar_entropy_token_count += 1
        ...
        next_token, next_token_entropy = next_text_token(outputs_cond.logits)
        ...
        while True:
            ...
            while True:
                token_item = next_token.item()
                record_selected_token_entropy(token_item, next_token_entropy)
                if token_item == eos_token_id or token_item == self.img_start_token_id:
                    break
                gen_tokens.append(token_item)
                generated_token_history.append(token_item)
            ...
            if next_token.item() == self.img_start_token_id:
                ...
                if self.noise_scale_mode in ("resolution", "dynamic", 'dynamic_sqrt'):
                    ...
                    if self.noise_scale_mode == 'dynamic_sqrt':
                        noise_scale = math.sqrt(noise_scale)
                noise_scale = min(noise_scale, self.noise_scale_max_value)
                image_prediction = noise_scale * torch.randn((1, 3, image_size[1], image_size[0]), device=device, dtype=outputs_cond.logits.dtype, generator=image_generator)
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False, grid_hw=None,
             ...
             enable_thinking=True, response_prefix=''):
        """enable_thinking: True(默认) 开启思维链；False 关闭思维链，直接输出答案。

        与 chat_template.jinja 中 `enable_thinking is defined and ... is false` 分支行为一致：
        关闭时在 assistant 起始处注入空 think 块 `<think>\n\n</think>\n\n`，使模型跳过思维直接作答。
        此前 chat() 未暴露该开关，导致 VQA 默认永远 think。

        ``response_prefix`` 只在关闭思维链时生效。它作为 assistant
        answer 区域的预填前缀，可防止部分 base 样本在空 think 块后
        立即生成 EOS。前缀本身不会重复出现在返回字符串中。
        """
        ...
        # 关闭思维链：对齐 chat_template 的 enable_thinking=False 行为，
        # 在 assistant 起始处注入空 think 块。
        if not enable_thinking:
            if response_prefix is None:
                response_prefix = ''
            if not isinstance(response_prefix, str):
                raise TypeError('response_prefix must be a string or None')
            query = query + '<think>\n\n</think>\n\n' + response_prefix
        ...
        # Preserve the decoded text before non-thinking stop-marker cleanup so
        # callers can diagnose empty outputs without re-running generation.
        self.last_chat_raw_response = response
        if not enable_thinking:
            for stop_string in generation_config.get('stop_strings', []):
                response = response.split(stop_string, 1)[0].strip()
            response = response.split('</think_never_used_', 1)[0].strip()
    ...
    def generate(
        ...
        if tokenizer is not None:
            generate_kwargs.setdefault('tokenizer', tokenizer)
