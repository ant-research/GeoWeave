...
class MultimodalRLDataSource:
    ...
    def _next_loader_batch(self) -> RolloutInputs:
    ...
    def get_samples(self, batch_size: int) -> RolloutInputs:
        """Return exactly ``batch_size`` prompts, preserving unused prefetched rows.

        The DataLoader keeps its efficient fixed ``prompts_per_rollout`` batch,
        while this driver-side buffer lets dynamic rollout request an initial
        batch plus reserve prompts without dropping the unused reserve rows.
        """
        requested = int(batch_size)
        if requested < 1:
            raise ValueError(f"batch_size must be >= 1, got {requested}")

        parts: List[RolloutInputs] = []
        remaining = requested
        while remaining > 0:
            if not self._sample_buffer:
                self._sample_buffer.append(self._next_loader_batch())
            head = self._sample_buffer.popleft()
            take = min(remaining, head.batch_size)
            parts.append(head.slice(0, take))
            if take < head.batch_size:
                self._sample_buffer.appendleft(head.slice(take, head.batch_size))
            remaining -= take
        return parts[0] if len(parts) == 1 else RolloutInputs.concat(parts)

    def return_samples(self, samples: RolloutInputs) -> None:
        """Put unused reserve prompts back at the front of the data stream."""
        if samples.batch_size > 0:
            self._sample_buffer.appendleft(samples)
