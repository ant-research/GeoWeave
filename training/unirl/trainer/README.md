...
`UnifiedModelTrainStack` also supports cross-rollout accumulation through
`stack.gradient_accumulation_steps`. `num_rollouts` keeps its existing meaning:
it is the total rollout/reward/backward and data budget, not the optimizer-step
count. With `num_updates_per_batch=1`, a fresh run performs
`ceil(num_rollouts / gradient_accumulation_steps)` optimizer steps; the final
short window is stepped on the last rollout. Configure scheduler `total_steps`
in optimizer-step units. Cross-rollout accumulation currently requires
`num_updates_per_batch=1`, eager FSDP gradient synchronization
(`defer_grad_sync=false`), and fixed per-track presence/batch size within each
window; variable tracks fail fast rather than silently changing AR/image loss
weights.
