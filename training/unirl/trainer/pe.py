...
class PETrainer(BaseTrainer):
    ...
    def train_step(
        ...
        *,
        ...
        self.metric_logger.log_rollout_step(rollout_id, results, resp, step_time_s=time.perf_counter() - t0)
    ...
    def train(self, *, num_rollouts: int, weight_sync_interval: int = 1) -> None:
        ...
        try:
            for rollout_id in range(num_rollouts):
                ...
                self.metric_logger.log_progress(rollout_id, num_rollouts, results, mean_reward, logger=logger)
        finally:
