...
class BaseTrainer:
    ...
    Also owns the rank-0 metric logger shared by every trainer. TensorBoard is
    the default backend; W&B remains available through ``logging.backend``.
    Subclasses call :meth:`_init_logging` once at the top of ``train``
    (it always builds a logger — a no-op null-object when reporting is off),
    then ``self.metric_logger.log_rollout_step(...)`` / ``log_progress(...)``
    after each ``train_step``, and :meth:`_finish_logging` in a ``finally``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Driver/rank-0 metric logger. Start with a disabled TensorBoard
        # null-object; _init_logging replaces it with the selected live backend.
        from unirl.utils.tensorboard_logger import UniRLTensorBoardLogger

        self.logging_cfg = logging_cfg
        self.logging_backend = "tensorboard"
        self.metric_logger = UniRLTensorBoardLogger(enabled=False)
        # Driver-side state from a resumed checkpoint's trainer_state.json
        # (logger run directory/id and step axis); populated by checkpoint load,
        # consumed by _init_logging. Empty for fresh runs.
        ...
        # until the subclass __init__ finishes, so install() runs in _init_logging.
    ...
    # ---- metric logging (shared by all v2 trainers) ------------------------
    ...
    def _init_logging(
        ...
        *,
        ...
        """Build the configured rank-0 metric logger.

        ``logging.backend`` accepts ``tensorboard`` (default) or ``wandb``. For
        backward compatibility, a legacy block containing ``report_to_wandb``
        and no explicit backend continues to select W&B. A present TensorBoard
        logging block is enabled by default; set ``report_to_tensorboard=false``
        to disable event writing.
        """
        cfg = self.logging_cfg or {}
        has_logging_block = self.logging_cfg is not None
        backend_raw = cfg.get("backend")
        if backend_raw is None:
            backend = "wandb" if "report_to_wandb" in cfg else "tensorboard"
        else:
            backend = str(backend_raw).strip().lower()
        if backend not in {"tensorboard", "wandb"}:
            raise ValueError(
                f"logging.backend must be 'tensorboard' or 'wandb', got {backend!r}"
            )
        ...
        run_config: Dict[str, Any] = {
            ...
            "samples_per_prompt": (
                total_samples_per_prompt(sampling_params)
                if sampling_params
                else None
            ),
        }
        ...
        resume_backend = self._resume_state.get("logging_backend")
        resume_run_id = self._resume_state.get("logger_run_id")
        if resume_backend and resume_backend != backend:
            resume_run_id = None
        ...
        if backend == "tensorboard":
            from unirl.utils.tensorboard_logger import init_tensorboard_logger

            enabled = has_logging_block and bool(
                cfg.get("report_to_tensorboard", cfg.get("enabled", True))
            )
            self.metric_logger = init_tensorboard_logger(
                **common_kwargs,
                enabled=enabled,
                run_id=resume_run_id,
            )
            if self.metric_logger.initialized:
                logger.info(
                    "TensorBoard initialized: log_dir=%s",
                    self.metric_logger.log_dir,
                )
        else:
            from unirl.utils.wandb_logger import init_logger

            enabled = (
                bool(cfg.get("report_to_wandb", False))
                and bool(project)
                and has_logging_block
            )
            # Old checkpoints only stored ``wandb_run_id``. Keep them resumable.
            wandb_run_id = resume_run_id or self._resume_state.get("wandb_run_id")
            self.metric_logger = init_logger(
                **common_kwargs,
    ...
    def _drop_decoded(
        ...
        *,
        ...
        """
        metric_log = getattr(self, "metric_logger", None)
        if metric_log is not None and metric_log.should_log_media(rollout_id):
            ...
            for name, track in resp.tracks.items():
                ...
                if preview is None and track.decoded is not None:
                    preview = build_media_preview_for_track(
                        req=req,
                        track=track,
                        max_items=metric_log.media_max_items,
                        prompts=prompts_by_track.get(name),
                    )
                if preview is None:
                    continue
                if len(preview) > metric_log.media_max_items:
                    preview = preview.slice(0, metric_log.media_max_items)
                key = f"rollout/{name}/generated_media" if multi else "rollout/generated_media"
                metric_log.log_generated_media(rollout_id + 1, preview, key=key)
    ...
    def _finish_logging(self) -> None:
        """Flush and close the active metric backend."""
    ...
    def maybe_save_checkpoint(
        ...
        *,
        ...
        logger.info(
            "[RUN][CHECKPOINT] step=%d/%d status=start path=%s",
            step,
            num_rollouts,
            path,
        )
        save_started = time.perf_counter()
        # Checkpoint saving may gather a full state_dict (torch format) or write
        # rank-local shards (DCP); bracket either path for memory diagnostics.
        if self._memory_monitor is not None:
        ...
        # Driver-owned logger state keeps the selected backend on the same
        # event directory/run and preserves the optimizer-step axis on resume.
        logger_state = {
            "logging_backend": self.logging_backend,
            "logger_run_id": self.metric_logger.run_id,
            "optimizer_step": self.metric_logger.optimizer_step,
        }
        if self.logging_backend == "wandb":
            logger_state["wandb_run_id"] = self.metric_logger.run_id
        with open(os.path.join(path, "trainer_state.json"), "w") as f:
            json.dump(logger_state, f)
        logger.info(
            "[RUN][CHECKPOINT] step=%d/%d status=done elapsed_s=%.1f path=%s",
            step,
            num_rollouts,
