...
class MemoryMonitor:
    ...
    def install(self, trainer: Any) -> None:
        ...
        Called from ``BaseTrainer._init_logging``. Wrapping is deferred until after
        ...
        for attr in ("stack", "backend"):
            ...
            if handle is not None and callable(getattr(handle, "get_memory_stats", None)):
                ...
                break
        trainer.metric_logger.memory_monitor = self
    ...
    def step_summary(self, step: Optional[int] = None) -> Dict[str, float]:
        ...
        if self._fallback is not None:
            ...
            self._fold(closing)
            # Peak metrics answer "how much did this step need"; the closing
            # readings answer "did live/cache/device memory keep growing after
            # the step finished", which is the signal needed for leak diagnosis.
            end_keys = {
                "allocated_gb": "end_memory_allocated_gb",
                "reserved_gb": "end_memory_reserved_gb",
                "device_used_gb": "end_device_memory_used_gb",
            }
            for src, dst in end_keys.items():
                values = [float(r[src]) for r in closing if src in r]
                if values:
                    self._step_max[dst] = max(values)
