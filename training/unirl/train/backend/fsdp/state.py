...
def clip_grad_norm(
    ...
    For DTensor grads (FSDP2 sharded), routes to the explicit local-sum
    + single all_reduce path. Standard ``torch.nn.utils.clip_grad_norm_``
    falls through to ``_dispatch_get_local_results_slow_path`` which
    allgathers each param's grad to a full tensor before computing its
    norm — O(num_params) collectives, minutes of wall-clock on a 40B
    model. Plain-tensor grads still take the standard path.
    """
    from torch.distributed.tensor import DTensor

    if any(isinstance(getattr(p, "grad", None), DTensor) for p in params):
