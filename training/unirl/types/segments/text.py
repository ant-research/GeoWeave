...
class TextSegment(Segment):
    ...
    # Character coordinates in the exact decoded string sent to the process
    # judge. Populated by the rollout pipeline and packed 1:1 with ``tokens``.
    decoded_char_starts: Optional[torch.Tensor] = packed_field(default=None)
    decoded_char_ends: Optional[torch.Tensor] = packed_field(default=None)

    # SCA signals. They remain None for ordinary sequence-reward GRPO.
    process_error_mask: Optional[torch.Tensor] = packed_field(default=None)
    raw_token_credit: Optional[torch.Tensor] = packed_field(default=None)
    sca_token_mask: Optional[torch.Tensor] = packed_field(default=None)
