...
class RolloutTrack(Batch):
    ...
    # Optional sample-aligned structured process judgments (SCA/GenPRM).
    process_annotations: Optional[List[Optional[Dict[str, Any]]]] = concat_field(default=None)

    # Conservative recovery metadata for max-length responses whose tail is an
    # exact generation loop. The packed segment is intentionally kept at its
    # original length for replay; ``TextSegment.loss_mask`` selects the recovered
    # prefix that is visible to SCA and contributes gradients.
    repetition_recovered: Optional[torch.Tensor] = concat_field(default=None)
    original_text_truncated: Optional[torch.Tensor] = concat_field(default=None)
    effective_text_token_count: Optional[torch.Tensor] = concat_field(default=None)
    removed_repetition_token_count: Optional[torch.Tensor] = concat_field(default=None)
    repetition_period_token_count: Optional[torch.Tensor] = concat_field(default=None)
    ...
    def compute_advantages(
        ...
        :param valid_mask: Optional per-sample 0/1 mask. Masked rows are excluded
            from the mean/std statistics and receive exactly zero advantage. This
            keeps fixed-shape rollout batches while allowing, for example, one
            truncated candidate in an 8-way group to leave a 7-way GRPO group.
        ...
        if valid_mask is not None:
            mask_local = hydrate(valid_mask).to(device=rewards_local.device).reshape(-1) > 0.5
            if int(mask_local.numel()) != n:
                raise ValueError(
                    f"compute_advantages: valid_mask length {int(mask_local.numel())} "
                    f"!= sample count {n}."
                )

        # Global scope: normalize across the whole batch, ignoring group
        # structure (reproduces v1 normalize_global). When a mask is supplied,
        # only valid rows contribute to the statistics and invalid rows stay zero.
        if scope == "global":
            rewards_g = rewards_local.to(torch.float32)
            selected = rewards_g if mask_local is None else rewards_g[mask_local]
            if selected.numel() == 0:
                adv_g = torch.zeros_like(rewards_g)
            else:
                mean_g = selected.mean()
                if normalize:
                    std_g = selected.std() + eps if selected.numel() > 1 else selected.new_tensor(eps)
                    adv_g = (rewards_g - mean_g) / std_g
                else:
                    adv_g = rewards_g - mean_g
                if mask_local is not None:
                    adv_g = adv_g * mask_local.to(dtype=adv_g.dtype)
        ...
        grouped_mask = (
            torch.ones_like(reshaped, dtype=torch.bool)
            if mask_local is None
            else mask_local.view(n_groups, branch)
        )
        mask_f = grouped_mask.to(dtype=reshaped.dtype)
        valid_counts = mask_f.sum(dim=1, keepdim=True)
        safe_counts = valid_counts.clamp_min(1.0)
        mean = (reshaped * mask_f).sum(dim=1, keepdim=True) / safe_counts
        centered = reshaped - mean
        if normalize:
            if use_global_std:
                # ``use_global_std``: per-group mean, but ONE batch-wide std
                # shared across valid rows. Keep the historical unbiased/Bessel
                # formula when at least two rows remain.
                selected = rewards[grouped_mask.flatten()]
                std = selected.std() + eps if selected.numel() > 1 else rewards.new_tensor(eps)
            else:
                # Population variance over only valid siblings. Empty and
                # singleton groups safely produce all-zero advantages.
                variance = (centered.square() * mask_f).sum(dim=1, keepdim=True) / safe_counts
