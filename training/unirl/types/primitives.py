...
class Images(Batch):
    ...
    When images have different sizes, ``from_list`` pads to the batch max and
    records per-image original ``(H, W)`` so ``to_list``/``to_pils`` can crop
    back to the true size.
    """

    pixels: torch.Tensor = field(kind=FieldKind.CONCAT, default=None)
    # Per-sample original (H, W) recorded when ``from_list`` pads the batch to
    # the max shape. Declared as a CONCAT list so DP scatter / slice / concat
    # keep it aligned with ``pixels`` (a plain attr would leak the full list to
    # every shard and break ``to_list`` after DP scatter).
    original_sizes: Optional[List[Tuple[int, int]]] = concat_field(default=None)
    ...
    def from_list(cls, items: List[Image]) -> "Images":
        ...
        original_sizes = [(int(p.shape[-2]), int(p.shape[-1])) for p in pixels_list]
        ...
        return cls(pixels=stacked, original_sizes=original_sizes)

    @classmethod
    def concat(cls, items: List["Images"]) -> "Images":
        """Concatenate image batches, padding spatial dims across batches."""
        if not items:
            raise ValueError("Cannot concat empty sequence of Images")
        if len(items) == 1:
            return items[0]
        if not all(isinstance(item.pixels, torch.Tensor) and item.pixels.ndim == 4 for item in items):
            return super().concat(items)

        max_h = max(int(item.pixels.shape[-2]) for item in items)
        max_w = max(int(item.pixels.shape[-1]) for item in items)
        padded_pixels = []
        original_sizes: List[Tuple[int, int]] = []
        needs_size_metadata = any(item.original_sizes is not None for item in items)

        for item in items:
            batch_size, _, height, width = item.pixels.shape
            if item.original_sizes is None:
                original_sizes.extend([(int(height), int(width))] * int(batch_size))
            else:
                original_sizes.extend(item.original_sizes)

            if height != max_h or width != max_w:
                needs_size_metadata = True
                pixels = torch.nn.functional.pad(
                    item.pixels,
                    (0, max_w - width, 0, max_h - height),
                    mode="constant",
                    value=0,
                )
            else:
        ...
        return cls(
            pixels=torch.cat(padded_pixels, dim=0),
            original_sizes=original_sizes if needs_size_metadata else None,
        )

    def to_list(self) -> List[Image]:
        if self.original_sizes is not None:
            return [
                Image(pixels=self.pixels[i, :, :h, :w])
