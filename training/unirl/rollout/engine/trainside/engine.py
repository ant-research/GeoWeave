...
class TrainsideRolloutEngine(BaseRolloutEngine):
    ...
    def generate_live(
        ...
        *,
        admit_fn: Callable[
            [int, bool], Tuple[List[Tuple[Any, tuple, dict]], bool]
        ],
        emit_fn: Callable[[Any, RolloutResp], None],
    ) -> None:
        """Run one persistent pipeline session with live request admission."""
        pipeline_generate_live = getattr(self.pipeline, "generate_live", None)
        if pipeline_generate_live is None:
            raise NotImplementedError(
                f"{type(self.pipeline).__name__} does not support generate_live"
            )

        def prepare_admitted(capacity: int, block_when_idle: bool):
            items, closed = admit_fn(capacity, block_when_idle)
            if self.schedule_policy is not None:
                for _, args, kwargs in items:
                    if kwargs or len(args) != 1 or not isinstance(args[0], RolloutReq):
                        raise TypeError(
                            "live rollout queue items must be positional RolloutReq calls"
                        )
                    ensure_req_sigmas(args[0], self.schedule_policy)
            return items, closed
        ...
        try:
            with torch.no_grad():
                pipeline_generate_live(
                    admit_fn=prepare_admitted,
                    emit_fn=emit_fn,
                )
        finally:
            for m, mode in zip(self._models, prev_modes):
