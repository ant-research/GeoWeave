...
class Worker:
    ...
    def _resolve_call_inputs(self, args: tuple, kwargs: dict):
        ...
        return map_tree(args, resolve), map_tree(kwargs, resolve)

    def _pack_call_result(self, result):
    ...
    def call(self, role_name: str, method_name: str, args: tuple, kwargs: dict, grad_mode: bool = False, call_id=None):
        ...
        resolved_args, resolved_kwargs = self._resolve_call_inputs(args, kwargs)
        ...
        return self._pack_call_result(result)


    def live_session_call(
        self, role_name: str, method_name: str, input_queue, output_queue
    ):
        """Run one long-lived model call with queue-backed row admission.

        The role method receives ``admit_fn(capacity, block_when_idle)`` and
        ``emit_fn(request_id, result)`` callbacks. Logical queue items use the
        same ``(request_id, args, kwargs)`` transport contract as
        :meth:`session_call`; ``None`` closes admission only after the model has
        drained its active rows. A list of queue items is admitted atomically;
        this is used for small prompt-affinity microbundles.
        """
        import queue as queue_module

        role = self._roles[role_name]
        current_request_id = None
        deferred_bundle = None

        def admit_fn(capacity: int, block_when_idle: bool):
            nonlocal current_request_id, deferred_bundle
            admitted = []
            closed = False
            capacity = max(0, int(capacity))
            while len(admitted) < capacity:
                if deferred_bundle is not None:
                    bundle = deferred_bundle
                    deferred_bundle = None
                else:
                    try:
                        item = input_queue.get(
                            block=bool(block_when_idle and not admitted)
                        )
                    ...
                    if item is None:
                        closed = True
                        break
                    # A list is one atomic admission unit. The scheduler uses
                    # this to keep a small same-prompt microbundle on one
                    # worker while preserving per-trajectory request IDs.
                    bundle = item if isinstance(item, list) else [item]

                remaining = capacity - len(admitted)
                if len(bundle) > remaining and not (block_when_idle and not admitted):
                    deferred_bundle = bundle
                    break
                for request_id, args, kwargs in bundle:
                    current_request_id = request_id
                    resolved_args, resolved_kwargs = self._resolve_call_inputs(
                        tuple(args), dict(kwargs)
                    )
                    admitted.append((request_id, resolved_args, resolved_kwargs))
            return admitted, closed

        def emit_fn(request_id, result):
            output_queue.put(
                ("ok", request_id, self._pack_call_result(result))
            )

        try:
            getattr(role, method_name)(admit_fn=admit_fn, emit_fn=emit_fn)
    ...
    def session_call(self, role_name: str, method_name: str, input_queue, output_queue):
        """Serve queued calls without re-entering the Ray actor per request.

        Queue items are ``(request_id, args, kwargs)`` tuples. ``None`` closes
        the session after all earlier items have completed. Results are packed
        through the same TensorTransport path as :meth:`call` before being put
        on the output queue.
        """
        role = self._roles[role_name]
        while True:
    ...
    def setup_global_pg(
        self, master_addr: Optional[str] = None, master_port: Optional[str] = None
    ) -> None:
        """Initialize the cross-worker transfer group (slot0 only).

        Not routed through transport_op: it injects Worker identity (rank /
        world_size) that the controller does not pass.  Rendezvous coordinates
        are also applied explicitly here so managed-cluster MASTER_ADDR/PORT
        values inherited by the Ray worker cannot shadow DevicePool's values.
        """
