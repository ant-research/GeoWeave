...
def _distributed_runtime_env_vars() -> Dict[str, str]:
    """Return NCCL settings that must be explicit in every Ray actor."""
...
class DevicePool:
    ...
    def _spawn_worker(self, device_id: int, slot: int, env_vars: dict = None) -> ray.actor.ActorHandle:
        ...
        runtime_env_vars = _distributed_runtime_env_vars()
        if env_vars:
            runtime_env_vars.update({str(key): str(value) for key, value in env_vars.items()})
        if runtime_env_vars:
            options["runtime_env"] = {"env_vars": runtime_env_vars}
    ...
    def _setup_nccl(self) -> None:
        ...
        # Pass rendezvous coordinates explicitly.  Some managed PyTorchJob
        # environments inject their own MASTER_ADDR/MASTER_PORT into every pod;
        # relying only on Ray runtime_env to override those inherited values can
        # make the actors connect to the platform rendezvous instead of this
        # DevicePool's TCPStore.
        ray.get(
            [
                w.setup_global_pg.remote(self._master_addr, self._master_port)
                for w in slot0_workers
            ]
        )
