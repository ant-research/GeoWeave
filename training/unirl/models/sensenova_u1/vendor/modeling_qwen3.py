...
def _compute_default_rope_parameters(config, device=None, **_kwargs):
    ...
    transformers 5.x additionally moved ``rope_theta`` from a direct attribute
    into a ``rope_parameters`` dict on the config; on 5.x, ``Qwen3Config(rope_theta=X)``
    stores it as ``config.rope_parameters['rope_theta']`` and drops the direct
    attribute. Fall through to that dict when the attribute is missing.
    """
...
class Qwen3RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, device=None):
        ...
        # transformers 5.x _init_weights looks up
        # ``module.compute_default_rope_parameters`` by attribute when
        # rope_type == "default". Alias to the vendored keep-freq-range wrapper
        # so re-initialization produces the same inv_freq computed below.
        self.compute_default_rope_parameters = self.rope_init_fn
...
class Qwen3Attention(nn.Module):
    ...
    def forward_und(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return_current_kv = bool(kwargs.pop("return_current_kv", False))
        ...
        if return_current_kv:
            return attn_output, attn_weights, current_key_states, current_value_states
        return attn_output, attn_weights
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward_und(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Self Attention
        return_current_kv = bool(kwargs.get("return_current_kv", False))
        ...
        if return_current_kv:
            hidden_states, _, current_k, current_v = attn_outputs
        else:
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        image_gen_indicators (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Boolean mask selecting image-generation tokens. Where `True`, the token is routed through
            the MoT `_mot_gen` branch (mot_gen final norm and MoT-suffix per-layer submodules); where
            `False`, it goes through the understanding branch. Defaults to all-`False` (pure text).
        indexes (`torch.LongTensor` of shape `(3, num_blocks)`, *optional*):
            Per-block index tuple driving block-causal masking during interleaved image/text generation.
            Row 0 tracks sequence position, rows 1/2 track per-image spatial (h, w) coordinates. Required
            when `attention_mask` is passed as a pre-built dict of masks (streaming decode / KV-cache
            resume); otherwise the model constructs it internally from `self.current_index`.
        cache_position (`torch.LongTensor` of shape `(sequence_length,)`, *optional*):
            Absolute positions of the current input tokens within the KV cache. Defaults to a contiguous
            range starting from `past_key_values.get_seq_length()`.
        """
        ...
        return_current_kv = bool(kwargs.pop("return_current_kv", False))
        if return_current_kv and use_cache:
            raise ValueError("return_current_kv=True requires use_cache=False")
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            if return_current_kv:
                hidden_states, current_k, current_v = layer_outputs
                functional_keys.append(current_k)
                functional_values.append(current_v)
            else:
        ...
        if not exist_image_gen_tokens:
            ...
            hidden_states = _hidden_states
        if return_current_kv:
            return hidden_states, tuple(functional_keys), tuple(functional_values)
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        indexes (`torch.LongTensor` of shape `(3, num_blocks)`, *optional*):
            Per-block index tuple forwarded to the underlying `Qwen3Model` to drive block-causal masking
            during interleaved image/text generation. See `Qwen3Model.forward` for the row layout.
        cache_position (`torch.LongTensor` of shape `(sequence_length,)`, *optional*):
            Absolute positions of the current input tokens within the KV cache. Defaults to a contiguous
            range starting from `past_key_values.get_seq_length()`.
