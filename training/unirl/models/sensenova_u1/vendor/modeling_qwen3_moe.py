...
class Qwen3MoeForCausalLM(Qwen3MoePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        indexes (`torch.LongTensor` of shape `(3, num_blocks)`, *optional*):
            Per-block index tuple forwarded to the underlying `Qwen3MoeModel` to drive block-causal
            masking during interleaved image/text generation. See `Qwen3MoeModel.forward` for the row
            layout.
        cache_position (`torch.LongTensor` of shape `(sequence_length,)`, *optional*):
            Absolute positions of the current input tokens within the KV cache. Defaults to a contiguous
            range starting from `past_key_values.get_seq_length()`.
