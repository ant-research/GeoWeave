...
class RewardService(Remote):
    ...
    def __init__(
        ...
        if self.truncated_reward == "soft" and self.overlong_buffer_len <= 0:
            raise ValueError(
                "overlong_buffer_len must be positive when truncated_reward='soft', "
                f"got {self.overlong_buffer_len}"
            )
        if self.truncated_reward == "soft" and self.overlong_penalty_factor < 0:
            raise ValueError(
                "overlong_penalty_factor must be non-negative when truncated_reward='soft', "
                f"got {self.overlong_penalty_factor}"
            )
        logger.info(
    ...
    def score_and_attach(self, *, req: RolloutReq, track: RolloutTrack) -> RolloutTrack:
        ...
        if self.truncated_reward != "keep" and ar_params is not None and track.segment is not None:
            ...
            if seg_lengths is not None and seg_lengths.numel() == rewards.numel():
                ...
                if self.truncated_reward == "zero":
                    ...
                    if self.overlong_buffer_len > int(max_len):
                        raise ValueError(
                            "overlong_buffer_len must not exceed AR max_new_tokens when "
                            f"truncated_reward='soft': buffer={self.overlong_buffer_len}, "
                            f"max_new_tokens={int(max_len)}"
                        )
                    ...
                    component_rewards["overlong_reward"] = penalty
                    component_rewards["overlong"] = (penalty < 0).to(torch.float32)
                    component_rewards["response_token_length"] = seg_lengths
        ...
        process_annotations = list(reward_response.process_annotations or [])
        if process_annotations and len(process_annotations) != len(sample_ids):
            raise RuntimeError(
                "RewardService.score_and_attach: process_annotations length "
                f"{len(process_annotations)} != sample count {len(sample_ids)}."
            )

        track = _track_with_field(track, "rewards", rewards)
        track = _track_with_field(track, "component_rewards", component_rewards)
        if process_annotations:
            track = _track_with_field(track, "process_annotations", process_annotations)
        return track
