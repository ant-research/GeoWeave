...
elif [ -n "${VENV_DIR:-}" ] && [ -f "${VENV_DIR}/bin/activate" ] \
    && [ "${SKIP_VENV_ACTIVATE:-0}" != "1" ]; then
    # shellcheck disable=SC1091
    source "${VENV_DIR}/bin/activate"
fi

# Resolve one authoritative Python interpreter for the driver, optional editable
# install, Ray CLI, and Ray workers.  Do not rely on a bare `python`: shared
# virtualenv activate scripts embed the absolute path at creation time and can
# silently redirect PATH when the same repository is mounted elsewhere.
...
[ -n "${PYTHON_BIN}" ] && [ -x "${PYTHON_BIN}" ] || {
    echo "Python executable not found; set PYTHON_BIN or VENV_DIR." >&2
    exit 2
}
export PYTHON_BIN

if [ -z "${RAY_BIN:-}" ]; then
    # Pin Ray to the same interpreter as the training driver even when the
    # selected environment has Ray's package but no bin/ray console script.
    # Each node creates its own /tmp wrapper, including SSH fan-out workers.
    RAY_CLI_DIR="${RAY_CLI_DIR:-/tmp/unirl-ray-cli}"
    mkdir -p "${RAY_CLI_DIR}"
    RAY_BIN="${RAY_CLI_DIR}/ray"
    printf '#!/usr/bin/env bash\nexec %q -m ray.scripts.scripts "$@"\n' \
        "${PYTHON_BIN}" > "${RAY_BIN}"
    chmod +x "${RAY_BIN}"
fi
RAY_BIN="${RAY_BIN:-$(command -v ray || true)}"
[ -n "${RAY_BIN}" ] && [ -x "${RAY_BIN}" ] || {
    echo "Ray CLI not found; set RAY_BIN or install Ray in the selected environment." >&2
    exit 2
}
export RAY_BIN
